...
const Course = ({ course, handelClickEvent }) => {
    return (
        <div className="card bg-base-100 shadow-xl">
            <figure><img src={course.image} alt={course.title} /></figure>
            <div className="card-body">
                <h2 className="card-title  text-left text-[.9rem] font-bold">
                    {course.title}

                </h2>
                <p className='text-sm text-gray-500 text-left'>{course.description.slice(0, 200)}</p>
                <div className='flex text-[#1C1B1B99] items-center justify-between '>
                    <div className='flex items-center gap-2'>
                        <div className=''><svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <div><h1>Price : {course.price}</h1></div>
                    </div>
                    ...
                    <div className='flex items-center gap-2 '>
                        ...
                        <div><h1>Credit : {course.Credit}hr</h1></div>
                    </div>
                </div>
                <button onClick={() => handelClickEvent(course)} className='btn bg-[#2F80ED] capitalize text-white mt-2 hover:text-gray-600'>Select</button>
            </div>
        </div>
    )
}
